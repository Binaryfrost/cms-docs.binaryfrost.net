...
const posts = await client.getCollectionEntries<BlogPost>(blogCollectionId);
...
<ApiRoute role="ADMIN" response="UserWithWebsites[]" />
...
<ApiRoute role="VIEWER" response="CollectionEntryWithTitle[]" />
